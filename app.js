...
function renderWorkerPnr(data) {
  ...
  workerPnrList.innerHTML = documents.map((document) => `
    <article class="worker-pnr-card">
      <div class="worker-pnr-icon" aria-hidden="true">📘</div>
      <div class="worker-pnr-copy">
        <strong>${escapeHtml(document.title || document.code || "Documento PNR")}</strong>
        <span>${escapeHtml(document.document_type || "PNR")} · v${escapeHtml(document.version || "1.0")}</span>
        ${document.summary ? `<p>${escapeHtml(document.summary)}</p>` : ""}
      </div>
      <button type="button" class="worker-pnr-open" data-pnr-id="${escapeHtml(document.id)}" data-pnr-url="${escapeHtml(document.document_url || "")}">Abrir</button>
    </article>
  `).join("");
...
function openAppSettings() {
  ...
  const accountCenter = profile?.control_center_name || effectiveControlCenterCode() || "Asignado según tu ubicación";
